...
public class ReadDeploySetServlet extends HttpServlet {
    ...
    public ReadDeploySetServlet() {
        try {
            ...
            com.smart.common.RSLogger.SetUpLogInfo(String.format("start service error step readSetUp ,%s", e.getLocalizedMessage()));
            com.smart.common.RSLogger.ErrorLogInfo(String.format("start service error step readSetUp ,%s", e.getLocalizedMessage()), e);
        }
        try {
            RSThreadPool.initialTheadPool();
        } catch (Exception e) {
            com.smart.common.RSLogger.SetUpLogInfo(String.format("start service error step initialCachedTheadPool ,%s", e.getLocalizedMessage()));
            com.smart.common.RSLogger.ErrorLogInfo(String.format("start service error step initialCachedTheadPool ,%s", e.getLocalizedMessage()), e);
        }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            RSLogger.SetUpLogInfo(String.format("reLoad db information error step readDBInformation,%s", e.getLocalizedMessage()));
            RSLogger.ErrorLogInfo(String.format("reLoad db information error step readDBInformation,%s", e.getLocalizedMessage()), e);
        }
    }
